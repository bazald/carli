...
def main():
  ...
  if len(filenames) == 1:
    ...
    while True:
      ...
      if not line or line == '':
        ...
        if regret:
          smith.append(float(split[val1]) - float(split[val0 + 1]))
        else:
          ...
          if not line or line == '':
            ...
            if first_handle:
              ...
              if first_group:
                ...
                xs.append(int(split[0]))
              if regret:
                y_min = float(split[val1]) - float(split[val0 + 0])
                y_avg = float(split[val1]) - float(split[val0 + 1])
                y_max = float(split[val1]) - float(split[val0 + 2])
              else:
              ...
              mul0 = y_count * mul1
              if regret:
                y_min = min(y_min, float(split[val1]) - float(split[val0 + 0]))
                y_avg = y_avg * mul0 + (float(split[val1]) - float(split[val0 + 1])) * mul1
                y_max = max(y_max, float(split[val1]) - float(split[val0 + 2]))
              else:
  ...
  if memory_plot:
    ...
    # lower right
    if regret:
      legend = pylab.legend(labels, [l.get_label() for l in labels], loc=1, handlelength=4.2, numpoints=2, prop={'size':4}, bbox_to_anchor=(-0.03,0.05,1,1))
    else:
    ...
    # lower right
    if regret:
      pylab.legend(labels, [l.get_label() for l in labels], loc=1, handlelength=4.2, numpoints=2)
    else:
