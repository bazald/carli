...
namespace Mountain_Car {
  ...
  void Agent::generate_rete(const Rete::Rete_Node_Ptr &parent) {
...

    {
      ...
      auto predicate = make_predicate_vc(feature->predicate(), Rete::WME_Token_Index(Position::index, 2), feature->symbol_constant(), root_action_data->rete_action.lock()->parent_left());
...

    {
      ...
      auto predicate = make_predicate_vc(feature->predicate(), Rete::WME_Token_Index(Velocity::index, 2), feature->symbol_constant(), root_action_data->rete_action.lock()->parent_left());
