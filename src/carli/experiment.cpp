...
namespace Carli {
   ...
   cout_bak(cout.rdbuf())
  {
    ...
    options.add(     make_shared<Option_Ranged<double>>("inverse-epsilon-episodic-increment", 0.0, true, numeric_limits<double>::max(), true, 0.0), "Inverse epsilon increment to be applied after each episode.");
    ...
    options.add(     make_shared<Option_Itemized>("split-test", set<string>({"catde", "policy", "value"/*, "utile"*/}), "catde"), "'catde' splits based on TD error. 'policy' splits when there's a change to the greedy action. 'value' splits on maximal value difference between fringe nodes. 'utile' splits on whether expected return is increased.");
    ...
    options.add(     make_shared<Option_Ranged<int64_t>>("concrete-update-count", 0, true, numeric_limits<int64_t>::max(), true, 0), "After this step, unrefinement will be prohibited for a node, 0 to disable.");
    ...
    options.add(     make_shared<Option_Ranged<double>>("split-probability", 0.0, true, 1.0, true, 1.0), "With what probability should an refinement criterion be tested?");
    options.add(     make_shared<Option_Ranged<double>>("unsplit-probability", 0.0, true, 1.0, true, 1.0), "With what probability should an unrefinement criterion be tested?");
    ...
    options.add(     make_shared<Option_Ranged<bool>>("rete-disable-node-sharing", false, true, true, true, false), "Disable sharing of rete beta nodes. Disable for purposes of performance comparison.");
  ...
  int64_t Experiment::take_args(int argc, const char * const * argv) {
    ...
    const auto seed = uint32_t(dynamic_cast<const Option_Ranged<int64_t> &>(options["seed"]).get_value());
