...
namespace Carli {
   ...
   cout_bak(cout.rdbuf())
  {
    ...
    options.add(     make_shared<Option_Itemized>("unsplit-test", set<string>({"none", "catde", "policy", "value"}), "none"), "'none' disables. catde' splits based on TD error. 'policy' splits when there's a change to the greedy action. 'value' splits on maximal value difference between fringe nodes.");
